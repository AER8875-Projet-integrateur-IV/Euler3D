...
void Partition::SolveElem2Part() {
	...
	_m_elem2NodeStart = {0, 5, 13, 19, 27};
	_m_elem2Node = {9, 13, 5, 14, 8, 12, 6, 7, 4, 9, 13, 5, 14, 10, 12, 9, 11, 4, 14, 0, 2, 3, 1, 10, 12, 4, 11};
...
void Partition::SolveElem2Node() {
	...
	_m_localNode2Global.reserve(15);
...
void Partition::SolveBorder() {
	// Initialsation
	int NELEM = 4;
	std::vector<int> element2ElementStart = {0, 5, 11, 16, 22};
	std::vector<int> element2Element = {1, 4, 5, 6, 7, 0, 2, 8, 9, 10, 11, 1, 3, 12, 13, 14, 2, 15, 16, 17, 18, 19};
	...
	for (int iPart = 0; iPart < _m_nPart; iPart++) {
		...
		for (int iElemLoc = 0; iElemLoc < finE - debutE; iElemLoc++) {
			...
			int debutV = element2ElementStart[iElemGlob];
			int finV = element2ElementStart[iElemGlob + 1];
			for (int elemGlobj = debutV; elemGlobj < finV; elemGlobj++) {
				// Récupération de l'élément voisin
				int jElemGlob = element2Element[elemGlobj];
