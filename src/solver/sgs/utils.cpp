...
float ** qftoQc(float** qf){
    ...
    for (int i=0; i<nx; i++) {
        for (int j=0; j<3; j++) {
            ...
            else if (j==1) lo_Qc[i][j]=qf[i][1]*qf[i][0];
            else if (j==2) lo_Qc[i][j]=(qf[i][2]/(g-1)+1.0/2.0*qf[i][1]*(pow(qf[i][0],2)));
...
float ** Qctoqf(float** Qc){
    ...
    for (int i=0; i<nx; i++)
        ...
        for (int j=0; j<3; j++) {
            ...
            else if (j==1) lo_qf[i][j]= Qc[i][0];
            else if (j==2) lo_qf[i][j]=(g-1)*(Qc[i][2]-1.0/2.0*Qc[i][0]*((Qc[i][1]/pow(Qc[i][0],2))));
