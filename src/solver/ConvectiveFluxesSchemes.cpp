	...
	for (int i = 0; i < nbEvaluatedCells; i++) {
		...
		for (int primVarIdx = 0; primVarIdx < 6; primVarIdx++) {
			...
			if (delta2 == 0) {
				...
				limiter = (1 / delta2) * ((std::pow(delta1_max, 2) + epsilon_sqrd) * delta2 + 2 * std::pow(delta2, 2) * delta1_max) / (std::pow(delta1_max, 2) + 2 * std::pow(delta2, 2) + delta1_max * delta2 + epsilon_sqrd);
			} else if (delta2 < 0) {
				double epsilon_sqrd = std::pow(K * std::pow(Metrics.getCellVolumes()[iElem], 1 / 3), 3);
				double delta1_min = MaxMinPrimVars[globalCounter * 3 + 1] - MaxMinPrimVars[globalCounter * 3 + 2];
				limiter = (1 / delta2) * ((std::pow(delta1_min, 2) + epsilon_sqrd) * delta2 + 2 * std::pow(delta2, 2) * delta1_min) / (std::pow(delta1_min, 2) + 2 * std::pow(delta2, 2) + delta1_min * delta2 + epsilon_sqrd);
			}
